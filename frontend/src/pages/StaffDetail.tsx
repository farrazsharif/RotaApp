...
import { Role, Training, ImportantDate, FitForWork, YesNo, User, Shift, roleLabel } from '../types';
...
export default function StaffDetail() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'Rota' && <RotaTab userId={user.id} staffName={`${user.firstName} ${user.lastName}`} />}
...
function RotaTab({ userId, staffName }: { userId: string; staffName: string }) {
  ...
  // Group by calendar day for display and printing.
