...
export default function CarePlan() {
  ...
  const { data: serviceUser, isLoading: userLoading } = useQuery({
  ...
  const saveMut = useMutation({
    mutationFn: () => carePlansApi.save(id, {
  ...
  function printPlan() {
    ...
    const html = `<!DOCTYPE html><html><head><title>Care Plan — ${esc(`${serviceUser.firstName} ${serviceUser.lastName}`)}</title>
      ...
      <h1>Care Plan</h1>
      <div class="sub">Printed ${esc(format(new Date(), 'dd MMM yyyy, h:mm a'))}</div>
  ...
  if (userLoading || planLoading || !serviceUser) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <button onClick={() => navigate(`/service-users/${id}`)} className="text-sm text-blue-600 hover:underline mb-2">← Back to Service User</button>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Care Plan — {serviceUser.firstName} {serviceUser.lastName}</h1>
      ...
      <div className="flex gap-3 pb-6">
        ...
        <button onClick={() => navigate(`/service-users/${id}`)} className="btn-secondary btn">Back</button>
        {isManager && (
