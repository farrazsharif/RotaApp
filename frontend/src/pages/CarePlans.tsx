...
import { printCarePlan, type CarePlanSchedule } from '../lib/carePlanPrint';
...
export default function CarePlans() {
  ...
  const deleteMut = useMutation({
    mutationFn: (serviceUserId: string) => carePlansApi.remove(serviceUserId),
    onSuccess: (_d, serviceUserId) => {
      qc.invalidateQueries({ queryKey: ['care-plans', 'summary'] });
      qc.invalidateQueries({ queryKey: ['care-plan', serviceUserId] });
    },
  });
  ...
  async function openPlanView(su: ServiceUser, autoPrint: boolean) {
    setBusy(su.id);
    try {
      const plan = await carePlansApi.get(su.id);
      let schedule: CarePlanSchedule = {};
      if (plan?.schedule) { try { schedule = JSON.parse(plan.schedule); } catch { schedule = {}; } }
      printCarePlan(su, {
        schedule,
        ...
        otherNotes: plan?.otherNotes || '', reviewDate: plan?.reviewDate || '',
      }, { autoPrint, createdAt: plan?.createdAt, updatedAt: plan?.updatedAt });
    } finally {
      setBusy(null);
    }
  }
  ...
  function handleDelete(su: ServiceUser) {
    if (!window.confirm(`Delete the care plan for ${su.firstName} ${su.lastName}? This clears all saved details but keeps the client.`)) return;
    deleteMut.mutate(su.id);
  }

  const term = search.trim().toLowerCase();
  // Drop clients whose care has ended (discharged / deceased) — they're no
  // longer on the caseload, so they don't need a care plan here.
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Open a client to view their care plan, or edit it</p>
