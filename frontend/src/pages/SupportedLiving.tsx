...
export default function SupportedLiving() {
  ...
  // Group by their supported-living scheme (the site).
  const grouped = useMemo(() => {
    const m = new Map<string, ServiceUser[]>();
    for (const c of clients) {
      const key = c.site?.name?.trim() || 'No scheme assigned';
      if (!m.has(key)) m.set(key, []);
      m.get(key)!.push(c);
    }
    return [...m.entries()].sort((a, b) => (a[0] === 'No scheme assigned' ? 1 : b[0] === 'No scheme assigned' ? -1 : a[0].localeCompare(b[0])));
  }, [clients]);
    ...
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <div className="card text-center py-10">
          ...
          <p className="text-sm text-gray-400 mt-1">Mark a site as a <span className="font-medium">supported-living scheme</span> (Settings → Sites), then assign clients to that site and they'll appear here.</p>
        </div>
      ) : (
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {list.map((c) => (
                  <button key={c.id} onClick={() => navigate(`/service-users/${c.id}`)} className="card text-left hover:shadow-md transition-shadow">
                    ...
                    {c.site?.housingProvider && <p className="text-xs text-gray-500 mt-1">🏘️ {c.site.housingProvider}</p>}
                  </button>
                ))}
              </div>
            </div>
