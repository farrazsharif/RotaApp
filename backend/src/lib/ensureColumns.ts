...
export async function ensureServiceUserColumns(prisma: any): Promise<void> {
  ...
  // "paper" historic entries for spot checks and reviews: a source marker so the
  // schedule can be seeded from checks/reviews already held on paper.
  try {
    await prisma.$executeRawUnsafe(`ALTER TABLE "SpotCheck" ADD COLUMN IF NOT EXISTS "source" TEXT NOT NULL DEFAULT 'form'`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "Review" ADD COLUMN IF NOT EXISTS "source" TEXT NOT NULL DEFAULT 'form'`);
    // Service user + supervisor signatures on the 6-week / quarterly review forms.
    await prisma.$executeRawUnsafe(`ALTER TABLE "Review" ADD COLUMN IF NOT EXISTS "serviceUserSig" TEXT`);
    await prisma.$executeRawUnsafe(`ALTER TABLE "Review" ADD COLUMN IF NOT EXISTS "supervisorSig" TEXT`);
  } catch { /* tables not created yet on this deploy; db push adds them with the column */ }
  ...
  // CarePlanVersion — immutable dated snapshots of a completed care-plan review.
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "CarePlanVersion" (
      "id"            TEXT PRIMARY KEY,
      "companyId"     TEXT,
      "serviceUserId" TEXT NOT NULL,
      "data"          TEXT NOT NULL,
  ...
  // RiskAssessmentVersion — immutable dated snapshots of a completed Risk
  // Assessment review. One shared table for the whole family, keyed by "type".
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS "RiskAssessmentVersion" (
      "id"            TEXT PRIMARY KEY,
      "companyId"     TEXT,
      "serviceUserId" TEXT NOT NULL,
      "type"          TEXT NOT NULL,
  ...
  `);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "RiskAssessmentVersion_companyId_idx" ON "RiskAssessmentVersion"("companyId")`);
  await prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS "RiskAssessmentVersion_serviceUserId_type_idx" ON "RiskAssessmentVersion"("serviceUserId", "type")`);
